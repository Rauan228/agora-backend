...
class AnswerComposer
{
    ...
    public function template(array $query, array $matches, array $stats = [], array $turn = []): string
    {
        ...
        if ($matches === []) {
            ...
            $parts[] = 'Можно уточнить категорию, размеры в мм или объём.';
        ...
        if (! empty($query['missing_slots'])) {
            ...
            $parts[] = 'Могу сравнить варианты или отсортировать по цене.';
        } else {
            $parts[] = 'Могу уточнить параметры или сравнить с другими вариантами.';
        }
    ...
    private function acknowledgement(array $query, array $turn): ?string
    {
        ...
        if ($kind === TurnInterpreter::KIND_RESTATE_KIT) {
            $catName = $this->categoryNames($query);
            $msg = $catName !== null
                ? "Собрал комплект заново: **{$catName}**."
                : 'Собрал комплект заново по названным позициям.';
            if ($switched !== []) {
                $msg .= ' С прошлого заказа не перенёс '.$this->labelList($switched).'.';
            }
