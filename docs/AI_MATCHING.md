...
| Комплект у одного завода | 2+ категории → поиск **по линиям**, сборка `order_plan`: если один поставщик закрывает все линии живыми офферами — рекомендуем одну заявку |

---

## Комплект / одна оптовая заявка (`order_plan`)

«Короб и лист» — не два независимых чата и не один котёл карточек. Это **один заказ из нескольких линий**.

1. Каждая категория ищется своим запросом. Размер короба не ставится листу.
2. Поставщик попадает в `full_cover`, только если у него есть оффер **на каждую** линию (не «у него вообще бывает лист»).
3. Рекомендация — один RFQ, если завод **сильно** закрывает все линии (≥75%). Иначе `order_plan.pack`: минимум 2–3 заявки (короб у А, лист+скотч+стрейч у Б), не 4 отдельных тикета. Слабые 50% на одной линии не побеждают две крепкие заявки.
4. Юридия и логистика (счёт, склад, МОQ на сумму) **не** зашиты: ассистент только говорит, что комплект закрывается каталогом.
...
"order_plan": {
  ...
  "recommended": {
    "kind": "full_cover",
    "supplier_name": "ПаллетПром",
    "covers": 2,
    "needed": 2,
    "reason": "…одна оптовая заявка…",
    "lines": [{ "slug": "corrugated-boxes", "covered": true, "offer": {} }]
  }
}
```

Одна категория → `"order_plan": { "multi": false }`. Смены темы («теперь только лист») по-прежнему `topic_switch`. «Ещё лист» / «короб и лист» — `add_line`, slug'и объединяются.

На проде это дешёвый PHP/SQL: +1 поиск на линию, без GPU. Качество упирается в то, что у завода реально заведены обе позиции.
...
| `add_line` | «ещё гофролист», «короб и лист» | slug добавляется в комплект, размеры короба **не** сбрасываются |
